...
public class CommandExecutor implements org.bukkit.command.CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String label, String[] stringArguments) {
        ...
        if(DEBUG) logger.info("Command used: " + stringArguments.length + " arguments -> /" + label + " " + Arrays.toString(stringArguments));
        Argument[] objectArguments = new Argument[stringArguments.length];
        {
            for(int i = 0; i < stringArguments.length; i++) {
                ...
                if(DEBUG) logger.info("Handled argument: " + (i + 1) + "/" + stringArguments.length + ": " + objectArguments[i].toString());
            }
        }
                ...
                if(declaredAnnotation.senderType().canHandleCommandSender(commandSender) && (additionalPermission.length() == 0 || commandSender.hasPermission(additionalPermission))) {
                    ...
                    if(requiredArguments.length > 0 && currentMethod.getReturnType().isAssignableFrom(HandleResponse.class) &&
...

                        {
                            ...
                            if(DEBUG) logger.info(currentMethod.getName() + " -> ** RESULT: " + integer + "/" + nearValue + " **");
                            if(integer != null && integer <= nearValue) {
                                ...
                                if(integer == 0) {
                                    if(DEBUG) logger.info(currentMethod.getName() + " -> ** perfect method: " + getMethodArguments(currentMethod.getParameterTypes()) + " **");
                    ...
                    } else if(DEBUG) logger.info(currentMethod.getName() + " -> player is incompatible with first argument or method is invalid (" + PLAYER_CLASS.getName() + ")");
                } else if(DEBUG) logger.info(currentMethod.getName() + " -> Player is incompatible or don't have permission");
            }
        }
        if(DEBUG) logger.info(TITLE.replaceAll("=", "*"));
    ...
    private Object getPlayerClass(CommandSender commandSender) {
        return (Player) commandSender; // TODO change here for your custom class
    }
